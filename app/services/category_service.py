...
class CategoryService(BaseService[Category]):
    ...
    def before_delete(self, id: UUID, **kwargs) -> bool:
        ...
        if category.user_id != user_id:
            logger.warning(f"Attempt to delete category with ID: {id} not owned by user with ID: {user_id}")
            raise HTTPException(status_code=403, detail=f"You do not own this category")
        
        return category

    def before_update(self, id: UUID, category_in: CategoryUpdate, **kwargs) -> Optional[Category]:
        # Baisc validation
