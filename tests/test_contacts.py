...
def ensure_primary_user(client, auth_headers):
    ...
    r = client.post(
        "/api/v1/users",
        json={"name": "Primary", "email": unique_email},
...
def test_create_contact_when_user_exists(client, auth_headers):
    ...
    second_token = jwt.encode(
        {"sub": str(uuid4())}, os.environ["JWT_SECRET_KEY"], algorithm="HS256"
    )
...
def test_get_contacts_and_detail(client, auth_headers):
    ...
    r = client.post(
        "/api/v1/contacts", json={"email": "charlie@example.com"}, headers=auth_headers
    )
    assert r.status_code == 200
    ...
    r = client.get(
        f"/api/v1/contacts/{contact_response['relationship_id']}", headers=auth_headers
    )
    assert r.status_code == 200
    detail = r.json()
