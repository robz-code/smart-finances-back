...
┌─────────────────────────────────────────────────────────────────────────────┐
│  ReportingService                                                            │
│  - get_total_balance, get_accounts_balance, get_balance_history              │
│  - Uses: AccountService, TransactionService, BalanceSnapshotRepository,      │
│          FxService                                                           │
└───────────────────────────────────────┬─────────────────────────────────────┘
                                        │
          ┌─────────────────────────────┼─────────────────────────────┐
          ▼                             ▼                             ▼
┌─────────────────────┐   ┌─────────────────────────────┐   ┌─────────────────┐
│ BalanceSnapshotRepo │   │ TransactionService          │   │ FxService       │
│ - get_latest_before │   │ - get_net_signed_sum_for_   │   │ - convert()     │
│   _or_on()          │   │   account()                 │   │ (stub: 1:1)     │
│ - get_by_account_   │   │                             │   │                 │
│   and_date()        │   │                             │   │                 │
│ - delete_future_    │   │                             │   │                 │
│   snapshots()       │   │                             │   │                 │
...
For `get_account_balance(account_id, as_of)`:

1. **If snapshot exists** for month of `as_of`:
   - `balance = snapshot.balance + net_transactions(snapshot_date + 1 day, as_of)`
2. **If no snapshot**:
   - Compute balance at start of month: `initial_balance + sum(transactions with date < month_start)`
...
| `app/repository/balance_snapshot_repository.py` | CRUD + `get_latest_before_or_on`, `delete_future_snapshots` |
...
| `app/services/reporting_service.py` | Added `get_account_balance`, `get_accounts_balance`, `get_total_balance`, `get_balance_history`; uses `BalanceSnapshotRepository`, `FxService` |
| `app/dependencies/reporting_dependencies.py` | Injects `BalanceSnapshotRepository`, `FxService` into `ReportingService` |
